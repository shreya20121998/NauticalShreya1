sap.ui.define(
  ...
  function (BaseController,History,MessageToast,JSONModel,MessageBox,bufferedEventHandler) {
    ...
    return BaseController.extend("com.ingenx.nauti.masterdashboard.controller.PortMaster", {
      onInit() {
        oView = this.getView();
  ...
  onBackPress: function () {
    ...
    if (aSelectedIds.length === 0 && !newEntryFlag) {
      ...
      let Portc = this.getView().byId("value").getValue().trim();
      let Portn = this.getView().byId("desc").getValue().trim();
      let Country = this.getView().byId("country").getValue().trim();
      let Countryn = this.getView().byId("cName").getValue().trim();

     
      if (Portc == "" && Portn == ""&&Country == "" && Countryn == "") {
  ...
  onPressHome: function () {
    ...
    if (aSelectedIds.length === 0 && !newEntryFlag) {
      ...
      pressEdit: function () {
        ...
        aSelectedItems.forEach(function (oItem) {
          ...
          console.log(sPortc,sPortn,sCountry,sCountryn);

          onEditInput.push(sPortc,sPortn,sCountry,sCountryn);
        });
        ...
        aSelectedItems.forEach(function (oSelectedItem) {
          ...
          let oColumnListItem = new sap.m.ColumnListItem({
            cells: [
              ...
              new sap.m.Input({ value: sCountry, editable: true}),
        ...
        oTable.getItems().forEach(function (row) {
            ...
            oBindListSP.attachEventOnce("dataReceived", function () {
                var existingEntries = oBindListSP.getContexts().map(function (context) {
                    return context.getProperty("Portc", "Portn","Country","Countryn").toUpperCase(); // Convert to lowercase
                });
    
                
                if (existingEntries.includes(value1),(value2),(value3),(value4)) {
    ...
    onUpdate: function () {
      ...
      for (let i = 0; i < aItems.length; i++) {
        let oItem = aItems[i];
        let sDesc = oItem.getCells()[[1],[2],[3],[4]].getValue();
      ...
      aItems.forEach(function (oItem) {
        ...
        let sDesc = oItem.getCells()[[1],[2],[3],[4]].getValue(); // Assuming Field Description is in the second cell
        console.log(aItems);
    ...
    onCancelEdit: function () {
      ...
      for (let i = 0; i < aItems.length; i++) {
        var oCells = aItems[i].getCells();
        var oInput = oCells[[1],[2],[3],[4]]; // Index 1 corresponds to the Input field
        var sValue = this.removeExtraSpaces(oInput.getValue());
    ...
    resetView: function () {
      ...
      this.getView().byId("cName1").setValue("");
